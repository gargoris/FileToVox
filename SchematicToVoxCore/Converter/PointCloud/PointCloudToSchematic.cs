...
namespace FileToVox.Converter.PointCloud
{
    public abstract class PointCloudToSchematic : AbstractToSchematic
    {
        ...
        protected void VoxelizeData(BodyDataDTO data)
        {
            ...
            Vector3 minX = data.BodyVertices.MinBy(t => t.X).First();
            Vector3 minY = data.BodyVertices.MinBy(t => t.Y).First();
            Vector3 minZ = data.BodyVertices.MinBy(t => t.Z).First();
            ...
            Vector3 maxX = data.BodyVertices.MaxBy(t => t.X).First();
            Vector3 maxY = data.BodyVertices.MaxBy(t => t.Y).First();
            Vector3 maxZ = data.BodyVertices.MaxBy(t => t.Z).First();

            Console.WriteLine("[INFO] Max X: " + maxX.X + ", Y: " + maxY.Y + ", " + maxZ.Z);

            minX = data.BodyVertices.MinBy(t => t.X).First();
            minY = data.BodyVertices.MinBy(t => t.Y).First();
            minZ = data.BodyVertices.MinBy(t => t.Z).First();
